...
describe("TileStore", () => {
  ...
  beforeEach(() => {
    ...
    tile = new Tile({id: "1", height: 1, x: 0, y: 0, z: 0})
    neighbor = new Tile({id: "1", height: 3, x: 0, y: 1, z: -1})
  })
  ...
  it("sends 'move' message to websocket", () => {
    ...
    expect(store.channel.socket.push.calledWith('move', { world_id: "1", coordinates: {x: 0, y: 0, z: 0}, range: 50 })).toBe(true)
  })

  it("sends 'showAll' message to websocket", () => {
    store.showAll()
    expect(store.channel.socket.push.calledWith('move', { world_id: "1", coordinates: {x: 0, y: 0, z: 0}, range: 10000 })).toBe(true)
  })
})
